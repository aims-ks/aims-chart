...
module.exports = class BasicChartView {
  ...
  constructor(options) {
    ...
    // this._options.yAxis.useZero - y-axis minimum set to zero instead of minimum value.
  ...
  _render(modelData) {
    ...
    if (_allData.length > 0) {
      ...
      modelData.forEach(function(model) {
        if (model.length > 0) {
          const tempBins = histogram(model);

          // Remove any empty bins.
          const bins = tempBins.filter(function(bin) { return bin.length > 0; });
      ...
      if (this._options.yAxis.single) {
        ...
        // By default, use calculated minimum.
        let yMin = minYvalue;
        // Only override in calculated minimum is greater than zero.
        if (minYvalue > 0) {
          // Override with zero from options?
          if (this._options.yAxis.useZero) {
            yMin = 0;
          } else {
            // Override by subtracting diff to max, but don't go below zero.
            yMin = yMin - yDiff;
            if (yMin < 0) {
              yMin = 0;
            }
