...
module.exports = class BasicChartView {
  ...
  _render(modelData) {
    ...
    if (_allData.length > 0) {
      ...
      // Build a scaler (xScale) and histogram (for data chunking) to handle data on the x-axis.
      // This requires considering ALL x data.
      const xScale = d3scale.scaleUtc()
        .domain(d3array.extent(_allXValues))
        .nice(this._options.xAxis.tickScale)
      ...
      modelData.forEach(function(model) {
        if (model.length > 0) {
          ...
          // Remove any empty bins at the end.
      ...
      if (this._options.yAxis.single) {
        ...
        const yScale = d3scale.scaleLinear()
          ...
          .domain([
            (minYvalue > 0 ? 0 : minYvalue),  // Use zero if all y values > 0.
            maxYvalue
          ]);
        this._canvas.append('g')
        ...
        yValues.forEach(function(yValue) {
          ...
          const selectedBins = [];
          renderer.render(yValue.bins, xScale, yScale, yValue.means, cssClass);
          index++;
        }, this);
