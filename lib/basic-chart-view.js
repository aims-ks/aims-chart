...
module.exports = class BasicChartView {
  ...
  constructor(options) {
    ...
    this._options.yAxis.single = this._options.yAxis.single || true;
  ...
  _render(models, dataReader) {
    ...
    if (_allData.length > 0) {
      ...
      const _allYValues = _allData.map(function (d) { return d.y; });
      ...
      if (this._options.yAxis.single) {
        ...
        // Find minimum and maximum values amongst ALL data.
      const minYvalue = d3array.min(_allYValues);
      const maxYvalue = d3array.max(_allYValues);
      ...
      // Process each model, building a bin (a grouping of data along the x-axis) for each model,
      // ignoring any models that have no data.
      let modelIndex = 0;
      _models.forEach(function(model) {
        if (model.length > 0) {
          const bins = histogram(model);
          const yValues = bins.map(function(bin) {
          ...
          const cssClass = (this._options.cssClasses && this._options.cssClasses[modelIndex] ?
            this._options.cssClasses[modelIndex] : '');
          const renderer = (this._options.renderers && this._options.renderers[modelIndex] ?
            this._options.renderers[modelIndex] : LineChartRenderer);
          renderer.render(canvas, bins, xScale, yScale, yValues, cssClass);

          /*
          // Single y-axis or one for each dataset?
          if (this._options.yAxis.single) {

            // Render the charts on the canvas.
            const primaryValues = [];
            defn.renderer.render(canvas, this._height, this._width, bins, xScale, yScale, yValues, 'primary-chart', this._dataReader);
          }
          */
          modelIndex++;
        }
